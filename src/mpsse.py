...
class MPSSE(object):
    ...
    def OpenUsbDev(self, mode, usb, dev):
        """
        Opens the specified FTDI device.
        Should be called if the __init__ mode is not None.
        ...
        """
        self.context = _mpsse.OpenUsbDev(mode, usb, dev)
        if self.context.open == 0:
            raise Exception(self.ErrorString())
        return MPSSE_OK
    ...
    def SetMode(self, endianess):
        ...
        """
        if _mpsse.SetMode(self.context, endianess) == MPSSE_FAIL:
            raise Exception(self.ErrorString())
        return MPSSE_OK
